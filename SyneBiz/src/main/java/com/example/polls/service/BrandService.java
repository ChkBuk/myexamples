...
public class BrandService {
    ...
    public PagedResponse<Brand> getAllBrands(int page, int size) {
        ...
        if(brands.getNumberOfElements() == 0) {
            return new PagedResponse<>(Collections.emptyList(), brands.getNumber(),
            		brands.getSize(), brands.getTotalElements(), brands.getTotalPages(), brands.isLast());
        }

        List<Brand> brandResponses=brands.getContent();
        return new PagedResponse<>(brandResponses ,brands.getNumber(),
        		brands.getSize(), brands.getTotalElements(), brands.getTotalPages(), brands.isLast());
    }
/*
...
/*
    ...
    private Map<Long, Long> getBrandUserVoteMap(UserPrincipal currentUser, List<Long> pollIds) {
    ...
    Map<Long, User> getPollCreatorMap(List<Brand> brands) {
        // Get Poll Creator details of the given list of polls
        List<Long> creatorIds = brands.stream()
                .map(Brand::getCreatedBy)
                .distinct()
                .collect(Collectors.toList());
	...
	public Brand createBrand(@Valid BrandRequest brandRequest) {
